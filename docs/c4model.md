...
- **Metal layer**: THREE Objective-C host translation units, `src/metal.m` (device,
  command buffers, kernel registration, per-op dispatch, decode),
  `src/metal_prefill.m` (chunked prompt prefill) and `src/metal_validate.m`
  (per-dispatch size/param validation and grid geometry), over the shared internal
  header `src/metal_internal.h`; plus TWO shader translation units, `src/kernels.metal` and
  ...
  three `.m` files are different in kind from the two `.metal` ones: they REALLY link, so
  each split promotes `static` helpers to external linkage and R1's byte-identity
  gate does not transfer (task R2, 2026-08-20, nine promotions; task R3, 2026-08-20,
...
| `src/metal.m` | (task R2, 2026-08-20: the chunked-prefill half moved to `src/metal_prefill.m`, leaving 3547 lines. Task R3, 2026-08-20: the per-dispatch validation trio moved to `src/metal_validate.m`, leaving 3207 lines, still over the ~2000-line guideline.) Metal device init, weight-buffer wrapping (mmap, no-copy; bf16/f32/Q8_0 sizing), per-weight matvec kernel dispatch (`matmul_kernel_for`), one command buffer per decode token; one command buffer per prefill chunk (`sg_gpu_prefill`, M5.6, now in `src/metal_prefill.m`) unless `sg_gpu_set_prefill_max_burst` splits the layer sweep across several (2026-08-15), kernel registration (the `KI_` enum in `src/metal_internal.h`, the `SG_KERNELS` table here, the size/param checks in `src/metal_validate.m` since task R3), `sg_gpu_current_alloc_bytes` (B2, `MTLDevice.currentAllocatedSize`). Registers itself as `sg_kv`'s allocation backend at init. Decode state: the DEFAULT full-attention KV cache is fp16, allocated through `sg_kv` as SEPARATE per-layer K/V buffers (M5.2, `SURGE_KV_DTYPE` env toggle); `SURGE_KV_DTYPE=f32` selects the original combined-buffer f32 path unchanged, kept so the M2 gate's oracle never moves. DeltaNet conv/S state stays on its pre-M5.1 ad hoc allocation for decode; prefill (M5.6) threads DeltaNet state through `sg_kv`'s conv/S carriers (so `g->kv` is now allocated for any DeltaNet model on the f16 path) and bridges the final state back into the ad hoc buffers when it finishes. `sg_gpu_prefill`'s chunk loop also carries the B8 prefill duty-cycle: `sg_gpu_set_prefill_rest` arms an optional idle sleep (`pf_sleep_ms`, `nanosleep`+EINTR-retry) between chunks once the accumulated GPU-busy time PLUS an estimate of the next chunk's would cross a work budget, so the GPU is yielded before the budget is blown rather than after. Its purpose is keeping the compositor alive, not dodging a firmware clock clamp (rationale corrected 2026-08-15, see the B8 block below). Alongside it, `sg_gpu_set_prefill_max_burst` bounds how long a SINGLE command buffer holds the GPU by splitting the layer sweep. Both are disabled by default and both are pure timing or pure submission-boundary changes (touching no buffer/state/accumulator), so neither changes computed output. |
| `src/metal_prefill.m` | (task R2, 2026-08-20) the chunked prompt prefill half of the Metal host layer, 826 lines total: 785 lines moved out of `src/metal.m` (782 of them byte-identical; the other 3 are one 1-line-to-3-line comment retarget at `src/metal_prefill.m:148-150`, and one blank line was inserted at `src/metal_prefill.m:284` between the two moved regions) plus a 41-line new file header, cut when `src/metal.m` reached 4641 against the ~2000-line guideline. Figures corrected in fix round 1 (2026-08-20); this row previously said "826 lines moved verbatim", which conflated the file size with the moved body. What moved: the two per-layer CHUNK encoders `enc_attn_prefill` (M5.4, full attention) and `enc_gdn_prefill` (M5.5, gated DeltaNet), and `sg_gpu_prefill` (M5.6) which drives them across every layer one command buffer per chunk, saves and restores the one-token decode scratch around chunk-sized scratch, and bridges the final DeltaNet state out of `sg_kv`'s carriers into `L->conv_buf`/`L->ssm`. Also the B8 duty cycle (`sg_gpu_set_prefill_rest`, `pf_sleep_ms`) and the command-buffer segmentation (`sg_gpu_set_prefill_max_burst`), both disabled by default and both pure timing or submission-boundary changes that touch no buffer, state or accumulator. THIS SEAM WAS CHOSEN BECAUSE NOTHING IN DECODE CALLS IT: `sg_gpu_forward`, `enc_attn` and `enc_gdn` stayed in `src/metal.m` and reach nothing here, so the traffic is one-way, into the shared helpers. Unlike task R1's shader split this could NOT be gated on binary identity (Objective-C translation units genuinely link), so the gate was the exact check counts: `make check` 87604/0 and `make debug` 83614/0, both unmoved from the parent. |
| `src/metal_validate.m` | (task R3, 2026-08-20) the per-dispatch validation and grid geometry of the Metal host layer, 360 lines total: a 311-line body grown from the 305 lines moved out of `src/metal.m` (282 of them byte-identical; the deviations are the `static` dropped from the three definitions, `sg_check_sizes`' continuation line re-indented by the seven columns `static` occupied, and four comment blocks retargeted at the files the names in them now live in) plus a 49-line new file header and include block. What moved: `sg_check_sizes` (the per-kernel buffer-size preconditions, every product guarded by `mul_ck`/`add_ck`), `sg_check_params` (the per-kernel preconditions that are not about sizes) and `sg_gpu_grid` (the `SG_K_*` kind to (threadgroups, threads) mapping). THE R3 SEAM RUNS THE OPPOSITE WAY FROM R2's: nothing here is called from `src/metal_prefill.m`, and all six call sites of the three functions stayed in `src/metal.m` (`sg_gpu_run_op` calls all three, the three split-K one-shots call `sg_check_params`, `sg_enc_op` calls `sg_gpu_grid`), so it is the MOVED code that lost `static`, not the code it calls. Two things crossed the other way and were deliberately NOT promoted: `buf_big_enough` became `static inline` in `src/metal_internal.h` (26 of its 29 call sites stayed behind and it is a two-instruction predicate), and the `SG_K_*` grid-kind enum moved to that header as a declaration, which creates no symbol in any object. `SG_KERNELS`, `SG_N_KERNELS`, `sg_kernel_desc` and the `_Static_assert` stayed in `src/metal.m`, where five functions read them. Gated like R2 on the exact check counts (`make check` 87604/0, `make debug` 83614/0, both unmoved) plus `nm -a` (exactly 3 new global symbols, 0 removed, no out-of-line copy of any intended-inline helper in any object) and a per-function disassembly comparison against the parent (55 of 59 common functions instruction-for-instruction identical, including `sg_gpu_forward` at 1408 instructions with `enc_attn`, `enc_gdn` and `enc_attn_splitk` inlined into it). |
| `src/metal_internal.h` | (tasks R2, R3 and R4) the ONLY declarations `src/metal.m`, `src/metal_prefill.m` and `src/metal_validate.m` share: `SG_TG`, the `KI_` kernel index enum, the `SG_K_*` grid-kind enum (R3), `sg_gpu_buf`, `buf_big_enough` (R3), `sg_gpu_layer`, `struct sg_gpu`, `sg_enc`, the `PARAMS` macro, and the helpers that cross the two seams. Two kinds of function live here and the difference is deliberate: the one-line accessors and bit casts on the per-element encode path (`bufof`, `offof`, `fbits`, `mul_ck`, `add_ck`) plus `buf_big_enough` are `static inline` DEFINITIONS so they keep inlining in every file and gain no external symbol, while the nine larger helpers (`sg_gpu_errf`, `sg_scratch_ensure`, `sg_gpu_elem_width`, `sg_gemm_kernel_for`, `sg_gpu_embed_row`, `sg_gpu_alloc_f32`, `sg_enc_op`, `sg_enc_kv_store`, `sg_enc_matmul`) defined in `src/metal.m`, and the three defined in `src/metal_validate.m` (`sg_check_sizes`, `sg_check_params`, `sg_gpu_grid`), are PROTOTYPES only: those really did lose `static`, gain external linkage and lose cross-translation-unit inlining, which is per-dispatch or per-allocation cost, never per-element. ALL TWELVE CARRY THE `sg_` PREFIX SINCE TASK R4 (2026-08-21); they were bare (`gpu_errf`, `enc_op`, `check_params`, ...) between R2 and R4, and `tools/check_metal_globals.sh` now freezes the unprefixed set at empty so a thirteenth cannot land bare. Nothing here is a copy of anything in `src/metal.m`; a helper with callers on one side only stays static in that file. Not a public header: `struct sg_gpu` holds `id<MTL...>` members, so only a `.m` built against the frameworks can include it. |
...
- **In progress (branch `feat/m3-m5`):**
  ...
  - Task P2.2 (`src/kernels.metal` + `src/metal.m` + `surge.h` + `tests/test_metal_ops.c`,
    ...
    `SG_K_ATTN` carries a single `*groups` count), `sg_check_params` rules shared by both,
    a `sg_check_sizes` routing rule, and `splitk_sizes()` guarding every byte count through
  ...
  - Task P2.3a MEASURED + Task P2.3 (`src/metal.m` + `surge.h` + `src/kernels.metal`
    ...
    which encodes the partial (hand-rolled 2D grid, x = split, y = head, since `sg_gpu_grid`    cannot carry two group dimensions) and then the combine into the SAME open command
