...
| Split-K GQA threadgroups | Gate (dated) | `docs/17082026_splitk_gqa_threadgroups.md` | Tasks P2.4 through P2.7 on `k_attn_decode_splitk_partial_gqa`: one threadgroup per GQA GROUP instead of per query head, its measured split policy (cap 256), the greedy-token gate where the two policies diverge, and P2.7's OCCUPANCY FLOOR of 128 threadgroups. The floor exists because collapsing `repeat` threadgroups into one divides the grid by `repeat`, so at short context the GQA kernel was measurably SLOWER than the per-head one it replaces (0.822x on the 27B at seq 2048). A threadgroup floor fits both real shapes where a seq threshold cannot, since the per-shape crossovers are 1.7x apart in seq and in the opposite order. `attn_splitk_gqa` is still OFF by default. |
