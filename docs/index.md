...
|---|---|---|---|
| Architecture (C4 model) | Architecture (evergreen) | `docs/c4model.md` | Source of truth for containers, components (per-file responsibilities), decode/prefill data flows, and built-vs-planned status (M0-M3 + M5 shipped; bench harness B1-B6 + B8 shipped, B7 pending, now unblocked by B8's prefill duty-cycle). |
