...
| Split-K GQA threadgroups | Gate (dated) | `docs/17082026_splitk_gqa_threadgroups.md` | Task P2.4: `k_attn_decode_splitk_partial_gqa`, one threadgroup per GQA group so each K/V element is read once instead of once per query head (4x on the 4B shape, 6x on the 27B); GATED ON HARDWARE 2026-08-17, byte-identical vs the per-head partial, 1.46x-1.74x faster. Task P2.5 (same doc, appended section): the GQA-specific split policy `splitk_gqa_n_splits` that fixes P2.4's one stated blocker, mean regret 0.43% / worst 2.73% measured vs the per-point optimum. Task P2.6 (same doc, "Residual gap" section rewritten): `SURGE_SPLITK_GQA_CAP` makes the cap runtime-settable so the two split policies diverge at a reachable seq, turning P2.5's margin argument into a mutation-proved `make check` subtest plus a real-model 4B greedy A/B with byte-identical `gen_ids`. `SURGE_ATTN_SPLITK_GQA` default stays OFF; flipping it is a separate decision none of the three tasks makes. |
