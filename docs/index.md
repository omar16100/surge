...
| Split-K GQA threadgroups | Gate (dated) | `docs/17082026_splitk_gqa_threadgroups.md` | Task P2.4: `k_attn_decode_splitk_partial_gqa`, one threadgroup per GQA group so each K/V element is read once instead of once per query head (4x on the 4B shape, 6x on the 27B). Written and compiled with the GPU busy, so NO hardware gate has been run: the byte-identity bar, the `SURGE_ATTN_SPLITK_GQA` switch (default off), the exact deferred gate commands, and what has to be true before the default flips. |
