 ...
 *
 *   - sg_check_sizes, the per-kernel buffer-size preconditions;
 *   - sg_check_params, the per-kernel preconditions that are not about sizes;
 *   - sg_gpu_grid, the kernel kind -> (threadgroups, threads) mapping.
 *
 ...
 * src/metal.m except sg_gpu_errf, which R2 had already promoted. What crosses is
 * the three functions BELOW: all six of their call sites (sg_gpu_run_op for
 * all three, the three split-K one-shots for sg_check_params, sg_enc_op for
 * sg_gpu_grid) stayed in src/metal.m, so it is these three that lost `static`.
 *
 ...
 * definitions, sg_check_sizes' continuation line re-indented by the seven columns
 ...
 * transfer to Objective-C: sg_check_sizes, sg_check_params and sg_gpu_grid have
 * external linkage now and can no longer be inlined into their callers. All
 * three are per-DISPATCH, never per-element: sg_check_sizes and sg_check_params run
 * once per one-shot sg_gpu_run_* call, which then commits a command buffer and
 * waits for the GPU, and sg_gpu_grid runs once per encoded dispatch, next to the
...
sg_err sg_check_sizes(const char *kernel, const sg_gpu_buf *a, const sg_gpu_buf *b,
    ...
    if (strcmp(kernel, "k_rmsnorm") == 0) {
         ...
         * own stride, guaranteed by sg_check_params' stride>=head_dim); b is the
         ...
         * sg_check_params enforces before this runs, so p[1]/32 is the exact
         ...
         * multiple of 32, which sg_check_params enforces before this runs (the
         ...
         * inside its own stride, guaranteed by sg_check_params' stride>=head_dim,
        ...
        return sg_gpu_errf("gpu: %s binds more device buffers than sg_gpu_run_op's "
                           "(a, b, out); use sg_gpu_run_attn_splitk_partial/_gqa/"
                           "_gqa_online/_combine", kernel);
    } else {
        return sg_gpu_errf("gpu: no size rule for kernel '%s'", kernel);
    }

    if (!ok) {
        return sg_gpu_errf("gpu: %s params describe a region that overflows 64 bits", kernel);
    }

    if (!buf_big_enough(a, need_a)) {
        return sg_gpu_errf("gpu: %s input a is %llu bytes, needs %llu", kernel,
                           (unsigned long long)(a ? a->nbytes : 0),
                           (unsigned long long)need_a);
    }
    if (want_b && !buf_big_enough(b, need_b)) {
        return sg_gpu_errf("gpu: %s input b is %llu bytes, needs %llu", kernel,
                           (unsigned long long)(b ? b->nbytes : 0),
                           (unsigned long long)need_b);
    }
    if (!buf_big_enough(o, need_o)) {
        return sg_gpu_errf("gpu: %s output is %llu bytes, needs %llu", kernel,
...
sg_err sg_check_params(const char *kernel, const uint32_t *p) {
    if (strcmp(kernel, "k_matvec_q8") == 0) {
        ...
        if (p[1] == 0 || p[1] % 32 != 0) {
            return sg_gpu_errf("gpu: k_matvec_q8 cols %u must be a nonzero multiple of 32", p[1]);
        }
    } else if (strcmp(kernel, "k_rope") == 0) {
        if (p[1] < 2 || p[1] > p[0] || p[1] % 2 != 0) {
            return sg_gpu_errf("gpu: k_rope rope_dim %u must be even and in [2, head_dim %u]",
                               p[1], p[0]);
        }
    } else if (strcmp(kernel, "k_rope_heads") == 0) {
        ...
        if (p[1] < 2 || p[1] > p[0] || p[1] % 2 != 0) {
            return sg_gpu_errf("gpu: k_rope_heads rope_dim %u must be even and in [2, head_dim %u]",
                               p[1], p[0]);
        }
        if (p[3] < p[0]) {
            return sg_gpu_errf("gpu: k_rope_heads stride %u is smaller than head_dim %u",
                               p[3], p[0]);
        }
    } else if (strcmp(kernel, "k_attn_decode") == 0) {
        if (p[1] == 0 || p[0] % p[1] != 0) {
            return sg_gpu_errf("gpu: k_attn_decode n_heads %u is not a multiple of n_kv_heads %u",
                               p[0], p[1]);
        }
        if (p[4] < p[2]) {
            return sg_gpu_errf("gpu: k_attn_decode q_stride %u is smaller than head_dim %u",
                               p[4], p[2]);
        }
    } else if (strcmp(kernel, "k_conv1d_step") == 0) {
        ...
        if (p[0] == 0) return sg_gpu_errf("gpu: %s N must be nonzero", kernel);
        if (p[1] == 0) return sg_gpu_errf("gpu: %s M must be nonzero", kernel);
        if (p[2] == 0) return sg_gpu_errf("gpu: %s K must be nonzero", kernel);
    } else if (strcmp(kernel, "k_matmul_q8") == 0) {
         ...
         * checked here (before sg_check_sizes divides by 32) for the same
         * reason: a truncated block count would silently undersize need_b. */
        if (p[2] == 0 || p[2] % 32 != 0) {
            return sg_gpu_errf("gpu: k_matmul_q8 K %u must be a nonzero multiple of 32", p[2]);
        }
    } else if (strcmp(kernel, "k_rope_chunk") == 0) {
        ...
        if (p[1] < 2 || p[1] > p[0] || p[1] % 2 != 0) {
            return sg_gpu_errf("gpu: k_rope_chunk rope_dim %u must be even and in [2, head_dim %u]",
                               p[1], p[0]);
        }
        if (p[2] == 0) return (sg_err){"gpu: k_rope_chunk heads must be nonzero"};
        if (p[3] < p[0]) {
            return sg_gpu_errf("gpu: k_rope_chunk stride %u is smaller than head_dim %u",
                               p[3], p[0]);
        }
         ...
         * in sg_check_sizes are u64-guarded. Reject rather than truncate. */
        ...
        if (p[0] == 0) return sg_gpu_errf("gpu: %s n_heads must be nonzero", kernel);
        if (p[1] == 0 || p[0] % p[1] != 0) {
            return sg_gpu_errf("gpu: %s n_heads %u is not a multiple of n_kv_heads %u",
                               kernel, p[0], p[1]);
        }
        if (p[2] == 0) return sg_gpu_errf("gpu: %s head_dim must be nonzero", kernel);
        if (p[4] < p[2]) {
            return sg_gpu_errf("gpu: %s q_stride %u is smaller than head_dim %u",
                               kernel, p[4], p[2]);
        }
        ...
        if (p[6] == 0) return sg_gpu_errf("gpu: %s n_splits must be nonzero", kernel);
...
void sg_gpu_grid(int kind, const uint32_t *p, uint64_t *groups, uint64_t *elems) {
    ...
    *elems = 0;
     ...
     * sg_check_sizes re-guards the byte counts with mul_ck regardless. */
